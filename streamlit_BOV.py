...
def main():
	...
	if choice == 'BOV':
		...
		scales = list(str_census['Chain Scale'].unique())
		chain_scale = st.sidebar.multiselect('Filter Comps by Chain Scale',str_census['Chain Scale'].unique(),default=scales)
		data = pd.DataFrame()
		hotel = pd.DataFrame()
		comps = pd.DataFrame()
		if submit:
		...
		if submit3:
			...
			st.write('### Full Dataset', comps.loc[(comps['Chain Scale'].isin(chain_scale))])
